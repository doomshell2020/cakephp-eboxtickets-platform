...
<h1 style="display:none;">Event Calander</h1>
...
<div id="content">
    ...
    <div id="header">
      ...
      <div id="groupBtn">
        <button class="btn btn-info previousBtn"><i class="fas fa-angle-left"></i></button>
        <button class="btn btn-info nextBtn"><i class="fas fa-angle-right"></i></button>
      </div>
    </div>
    <div id="weekdays"></div>
    <div id="calendar"></div>
  ...
  <div class="card text-center newEventModal" id ="selected_event_data" style= "display:block;"> </div>
  <!-- <div class="card text-center deleteEventModal">
    <div class="card-title"><h3>- <i class="far fa-eye"></i> Event -</h3></div>
    <form>
  ...
  for(let i=0;i<days01.length;i++) {
    ...
    if(day01 > 0) {
      ...
      daySquare.setAttribute('data-date',dayString);
        ...
        $.ajax({
        async:true,
        data:{'selecteddate': dayString },
        dataType:"html",
        type:"POST",
        url:"<?php echo SITE_URL ;?>calendar/eventcheck",
        success:function (datadddd) {
        if(datadddd == '1'){
          daySquare.classList.add("eventdate_data");
        }else{

        }
        },
        });
        //event date check  end

        
      // daySquare.setAttribute("class","eventdate_data");
...
$( document ).ready(function() {
  ...
  $.ajax({
async:true,
data:{'selecteddate': current_date },
dataType:"html",
type:"POST",
url:"<?php echo SITE_URL ;?>calendar/selectdate",
success:function (data) {
$("#selected_event_data").html(data); 
},
});
});
