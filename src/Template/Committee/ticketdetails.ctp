...
<section id="Committee">
    <div class="container">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody align="center">
                    <?php if (!empty($ticketstype)) {
                        // $eventoffice_ticketssold = 0;
                        foreach ($ticketstype as $key => $value) {
                            // pr($value);
                            // exit;
                            if ($value['ticket_id'] != 0) {
                                // $getdata = $this->Comman->ticketdetails($value['ticket_id']);
                                $eventoffice_ticketssold = $this->Comman->ticketsalecount_committee($value['event_id'], $value['ticket_id']);
                                $total_left = $value['count'] - $eventoffice_ticketssold;
                            }
                            if ($total_left == 0) {
                                continue;
                            }
                    ?>
                    <tr>
                        <th><?php echo $total_left; ?></th>
                        <td><?php echo $value['eventdetail']['title']; ?></td>
                        <td>
                            <?php echo $eventdetails['currency']['Currency_symbol']; ?>
                            <?php echo ($value['eventdetail']['price']) ? sprintf('%.2f', $value['eventdetail']['price']) : "0.00"; ?>
                            <?php $eventdetails['slug']; ?> <?php echo $eventdetails['currency']['Currency']; ?>
                        </td>
                    </tr>
                    <?php }
                    } else { ?>
        ...
        <div class="count_box mt-4">
            <h4>Share Link</h4>
            <p>This link will have you as the committee member pre-selected so your friends can request tickets faster!
                Click the link to copy it now.</p>
            <div class="d-flex align-items-center input_links">
                <?php $user_id = $this->request->session()->read('Auth.User.id'); ?>
                <input type="URL"
                    value="<?php echo SITE_URL . 'event/' . $eventdetails['slug']; ?>/<?php echo $user_id; ?>"
                    id="sharelinkcopytext" class="form-control" readonly>
                <!-- <a href=""><?php //echo SITE_URL . 'event/' . $eventdetails['slug']; 
                                ?></a> -->
                <a href="javascript:void(0)" onclick="myFunction()" id="copied_check" title="Copy">
                    <img class="copy_icon" src="<?php echo SITE_URL; ?>images/link-icon.png" alt="">
                </a>
                <!-- <span class = "copied_check" style = "display:none;"></span>       class = "bi bi-clipboard"      -->
        ...
        <script>
        function myFunction() {
            // $('.copied_check').css("display","none");
            var copyText = document.getElementById("sharelinkcopytext");
            copyText.select();
            copyText.setSelectionRange(0, 99999);
            navigator.clipboard.writeText(copyText.value);
            // $("#copied_check").removeClass('bi bi-clipboard');
            // $("#copied_check").addClass('bi bi-clipboard-fill');
            $('#copied_check').prop('title', 'Link copied');
            // $('.copied_check').css("display","");
        ...
        <div class="count_box mt-4">
            <h4>Ticket Push</h4>
            <p>This tool allows you to 'push' approvals to a patron's cart using their eboxtickets email address. If the
                ticket being 'pushed' is free, it will go straight to their 'My Tickets'. You cannot undo this
                operation!</p>


            <form method="post" action="<?php echo SITE_URL; ?>committee/ticketpush">
                <div class="row g-3 form_bg">
                    ...
                    <div class="col-md-8">
                        <input type="text" class="form-control search user-typeahead" required name="email"
                            id="inlineFormInputGroup" placeholder="Search" data-provide="typeahead" autocomplete="off">
                    ...
                    <?php if (!empty($ticketstype)) {
                        $total_ticket_data = 0;
                        foreach ($ticketstype as $key => $valuepush) { //pr($valuepush);
                    ...
                    <div class="col-md-4 ">
                        <label
                            for=""><?php echo $valuepush['ticket_id'] == 0 ? "Comps" : $valuepush['eventdetail']['title']; ?>
                            <?php echo $eventdetails['currency']['Currency_symbol']; ?><?php echo sprintf('%0.2f', $valuepush['eventdetail']['price']); ?>
                            <?php echo $eventdetails['currency']['Currency']; ?>
                            (<?php echo $total_ticket_data; ?>)</label>
                    </div>
                    ...
                    <div class="col-md-8">
                        ...
                        <select id="question_type_id"
                            name="ticket_count[<?php echo $valuepush['eventdetail']['id']; ?>]"
                            class="form-select rooms <?php if ($valuepush['eventdetail']['type'] == "committee_sales") {
                                                                                                                                                                echo "commiteesalesticket";
                                                                                                                                                            } ?>"
                            data-val="<?php echo $valuepush['eventdetail']['title']; ?>"
                        ...
                        <div class="<?php echo "eventquestion_" . $ticket_name; ?>" style="display:none">
                            ...
                            <div class="<?php echo "question_" . $ticket_name; ?>">
                                <p class="attendee_no">Attendee #<span
                                        class="<?php echo "questioncount_" . $ticket_name; ?>" id="room-1">1</span></p>
                                <div class="<?php echo "subquestioncount_" . $ticket_name; ?>" id="subquestion-1">
                                    ...
                                    <div class="row mb-3">
                                        <label for="inputName"
                                            class="col-sm-5 col-6 col-form-label"><?php echo $find_question_value['question']; ?></label>
                                        <?php $find_question_items = $this->Comman->findquestionitems($find_question_value['id']); ?>

                                        <div class="col-sm-7 col-6">
                                            <input type="hidden" class="subquestionid_<?php echo $ticket_name; ?>"
                                                name="questionid<?php echo $ticket_name; ?>_1[]"
                                                value="<?php echo $find_question_value['id']; ?>">
                                            <?php if ($find_question_value['type'] == "Select") { ?>
                                            <select id="inputState"
                                                class="form-select subquestion_<?php echo $ticket_name; ?>"
                                                name="question<?php echo $ticket_name; ?>_1[]">
                                            ...
                                            <select id="inputState"
                                                class="form-select subquestion_<?php echo $ticket_name; ?>"
                                                name="question<?php echo $ticket_name; ?>_1[]" multiple>
                                                <?php foreach ($find_question_items as $find_question_value) { ?>
                                                <option value="<?php echo $find_question_value['items']; ?>">
                                                    <?php echo $find_question_value['items']; ?></option>
                                                <?php } ?>
                                            ...
                                            <input type="text"
                                                class="form-control subquestion_<?php echo $ticket_name; ?>"
                                                name="question<?php echo $ticket_name; ?>_1[]">
                                            <?php } ?>
                                            ...
                                            <div class="form-group form-check">
                                                <input type="checkbox" value="Agree"
                                                    class="form-check-input subquestion_<?php echo $ticket_name; ?>"
                                                    id="exampleCheck1" name="question<?php echo $ticket_name; ?>_1[]">
